...
[Tweak("oldDecorations", Priority = 10)]
public class OldDecorations : Tweak<OldDecorations.SettingsClass> {
	[SyncTweak] public static OldDecorations Instance { get; set; }


	public class SettingsClass : TweakSettings {
		[Draw("editorPlus.oldDecorations.hideTab")]
	...
	public static class HideDecorationTab {
		public static void Postfix(InspectorTab __instance, LevelEventType type, InspectorPanel panel) {
			if (type == LevelEventType.DecorationSettings && Settings.HideDecorationsTab)
	...
	public static class EnableDecorationFloor {
		public static bool Prefix(bool firstTime, scnEditor __instance) {
			var eventButtons = __instance.get<Dictionary<LevelEventCategory, List<LevelEventButton>>>("eventButtons");
			...
			using (new SaveStateScope(scnEditor.instance, false, false, false)) {
				...
				if (eventType != LevelEventType.None) {
					...
					if (eventType == LevelEventType.SongSettings) {
						...
						DebugUtils.Log("selectedFloorEvents.Count: " + selectedFloorEvents.Count);
					...
					foreach (var component in from object obj in __instance.tabs
						...
						if (component != null) {
							if (eventType == component.levelEventType) {
								...
								DebugUtils.Log("selected tab: " + component.levelEventType);
	...
	[HarmonyPatch(typeof(scnEditor), "SelectDecoration", typeof(LevelEvent), typeof(bool), typeof(bool), typeof(bool),
		typeof(bool))] // 장식 선택시 타일도 선택
	public static class SelectDecorationTile {
		...
		public static void Postfix(PropertyControl_List __instance) {
			__instance.OnItemSelected =
				(Action<LevelEvent>)info.CreateDelegate(typeof(Action<LevelEvent>), scnEditor.instance);
			;
		}

		public static MethodInfo info =
			typeof(scnEditor).GetMethod("OnDecorationSelected", BindingFlags.Instance | BindingFlags.NonPublic);
	}
